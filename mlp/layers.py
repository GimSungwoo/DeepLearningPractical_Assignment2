...
class ConvolutionalLayer(LayerWithParameters):
    ...
    def fprop(self, inputs):
        ...
        outputs = np.zeros((batch_size, num_output_channels, output_height, output_width))
        num_input_channels = self.num_input_channels
        
        for batch in range(batch_size):
            for num_output_channel in range(num_output_channels):
                for num_input_channel in range(num_input_channels):
                    kernel = self.kernels[num_output_channel, num_input_channel, :, :]
                    outputs[batch,num_output_channel,:,:] += conv2d(inputs[batch,num_input_channel,:,:], kernel, mode='valid')
                outputs[batch,num_output_channel,:,:] += self.biases[num_output_channel]
        ...
        for batch in range(batch_size):
            for num_input_channel in range(self.num_input_channels):
                for num_output_channel in range(self.num_output_channels):
                    kernel = self.kernels[num_output_channel, num_input_channel, ::-1, ::-1]
                    grads_wrt_inputs[batch,num_input_channel,:,:] += conv2d(grads_wrt_outputs[batch,num_output_channel,:,:], kernel)
        ...
        for num_output_channel in range(num_output_channels):
            for num_input_channel in range(num_input_channels):
                for batch in range(batch_size):
                    ...
                    grads_wrt_weights[num_output_channel, num_input_channel, :, :] += conv2d(inputs[batch,num_input_channel,:,:], kernel, mode='valid')
...
class MaxPooling2DLayer(Layer):
    ...
    def fprop(self, inputs):
        ...
        batch_size = inputs.shape[0]
        output_height, output_width = int((self.input_height - self.size)/self.stride + 1), int((self.input_width - self.size)/self.stride + 1)
            ...
            for num_output_channel in range(num_output_channels):
                for row in range(output_height):
                    for col in range(output_width):
                        max_pool = inputs[batch,num_output_channel,row*self.stride:row*self.stride+self.size,col*self.stride:col*self.stride+self.size].max()
                        outputs[batch,num_output_channel,row,col] = max_pool
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        for batch in range(batch_size):
            for num_output_channel in range(num_output_channels):
                for row in range(output_height):
                    for col in range(output_width):
                        mask = inputs[batch,num_output_channel,row*self.stride:row*self.stride+self.size,col*self.stride:col*self.stride+self.size]
                        max_index = np.unravel_index(mask.argmax(), mask.shape) 
                        grads_wrt_inputs[batch,num_output_channel,row*self.stride+max_index[0],col*self.stride+max_index[1]] = grads_wrt_outputs[batch,num_output_channel,row,col]
